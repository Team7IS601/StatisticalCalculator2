...
class MyTestCase(unittest.TestCase):
    test_data = getFileData('Tests/Data/Test_Data.csv').data
    print(test_data)
    column1 = [int(row['Value 1']) for row in test_data]
    column2 = [int(row['Valye 2']) for row in test_data]
    p_answers = getFileData('Tests/Data/Test_Proportion.csv').data
    z_answers = getFileData('Tests/Data/Test_ZScores.csv').data
    column_proportion = [float(row['Proportion']) for row in p_answers]
    column_zscore = [float(row['Z-Score']) for row in z_answers]
    test_answer = getFileData('Tests/Data/UnitTestStatsAnswers.csv').data
    sample_data = getFileData('Tests/Data/Test_Data_Sample.csv').data
    column3 = [int(row['Sample 1']) for row in sample_data]
    sample_answer = getFileData('Tests/Data/UnitTestSampleAnswers.csv').data
